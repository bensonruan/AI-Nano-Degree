...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # If A node has prenodes that are a subset of prev S level
    ...
    def add_literal_level(self, level):
        ...
        # Start new S level - empty
        s_level = set()
        # Get parent A level and loop through its nodes
        a_level = self.a_levels[level - 1]
        for a_node in a_level:
            # Get action's effects and connect the nodes
            eff_nodes = a_node.effnodes
            # Connect nodes
            for s_node in eff_nodes:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        pos1 = set(node_a1.action.precond_pos)
        neg1 = set(node_a1.action.precond_neg)
        add1 = set(node_a1.action.effect_add)
        rem1 = set(node_a1.action.effect_rem)
        pos2 = set(node_a2.action.precond_pos)
        neg2 = set(node_a2.action.precond_neg)
        add2 = set(node_a2.action.effect_add)
        rem2 = set(node_a2.action.effect_rem)
        return (pos1 & rem2) | (neg1 & add2) | (pos2 & rem1) | (neg2 & add1)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            for a2 in node_s2.parents:
                if not a1.is_mutex(a2): return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for clause in self.problem.goal: 
            goalNode = PgNode_s(clause, True);
            for level_number,s_level in enumerate(self.s_levels):
                if goalNode in s_level :
                    level_sum = level_sum + level_number;
                    break; # stop after first appearance
